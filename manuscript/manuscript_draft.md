...
Independent high-accuracy sequence evidence was used to construct a locus-level truth set. Truth states were classified as HIGH_CONFIDENCE_HIFI_TRUTH, AMBIGUOUS_HIFI, or UNCALLABLE_HIFI. Short-read concordance was evaluated only in the high-confidence comparison universe.
...
The frozen pilot contrasts were small. PRJNA360417 contained 5 NAG and 6 IM patients; PRJNA678459 contained 5 AG and 5 GC patients; PRJNA1103397 contributed six GC isolates but no internal disease contrast (Figure 4B).
...
The public-cohort stress test also provides an important negative result. The original biological question was whether reproducible phase-variable functional states associate with progression across gastric carcinogenesis after accounting for population structure and cohort effects. The available public data did not permit that question to be tested rigorously under the frozen rules. The limiting factors were not a lack of nominal p-values but insufficient jointly callable loci, small within-study disease contrasts, confounding between disease labels and genomic structure, and broader coupling between study, geography, and sequencing context.
...
(A) Single-country versus multicountry distribution among 45 Correa-labelled studies. (B) Pilot cohort sample sizes. (C) Locus-screen attrition. (D) Strict endpoint status. Both within-cohort disease contrasts are NOT_COMPUTABLE under the pre-specified joint-callability floor; displayed permutation values are diagnostic only.
